function parseUplink(device, payload) {
  ...
  // Cumulative flow
    if (decoded.some(variable => variable.variable === 'cumulative_flow')) {
        ...
        if (sensor3 != null)
            sensor3.updateFlowSensorValueUnits(cflowValue);
    };
  // Reverse cumulative flow
    if (decoded.some(variable => variable.variable === 'reverse_cumulative_flow')) {
        ...
        if (sensor4 != null)
            sensor4.updateFlowSensorValueUnits(rflowValue);
    };
  // Daily cumulative flow
    if (decoded.some(variable => variable.variable === 'daily_cumulative_amount')) {
        ...
        if (sensor5 != null)
            sensor5.updateFlowSensorValueUnits(dflowValue);
    };
...
function Decoder(bytes) {
    ...
    const flow_rate = parseInt(readInt32LE(bytes, 30).toString('16')) / 10000;
    ...
    /*const alarm = readInt16BE(bytes, 44).toString(2).padStart(16, '0');
